...
public class OpCreateSessionActionTest {
    ...
    public static ConnectResponse defaultResponse(ConnectRequest request) {
        return new ConnectResponse(request.getProtocolVersion(),
                request.getTimeOut(), request.getSessionId(),
                request.getPasswd());
    }

    @Test
    public void testRequestSerialization() throws IOException {
        OpCreateSessionAction.Request input = OpCreateSessionAction.Request
                .create().setRequest(defaultRequest()).setReadOnly(false)
                .setWraps(false);
        ...
        OpCreateSessionAction.Request output = OpCreateSessionAction.Request
                .create().decode(inStream);
        inStream.close();
    ...
    public void testResponseSerialization() throws IOException {
        OpCreateSessionAction.Response input = OpCreateSessionAction.Response
                .create().setResponse(defaultResponse(defaultRequest()))
                .setReadOnly(false).setWraps(false);
        ByteArrayOutputStream outStream = new ByteArrayOutputStream();
        input.encode(outStream);
        outStream.close();
        ...
        ByteArrayInputStream inStream = new ByteArrayInputStream(
                outStream.toByteArray());
        OpCreateSessionAction.Response output = OpCreateSessionAction.Response
                .create().decode(inStream);
        inStream.close();
        assertEquals(input, output);
    }
    ...
    public void testResponseCompatibility() throws IOException {
        OpCreateSessionAction.Response input = OpCreateSessionAction.Response
                .create().setResponse(defaultResponse(defaultRequest()))
                .setWraps(true).setReadOnly(false);
        ByteArrayOutputStream outStream = new ByteArrayOutputStream();
        input.encode(outStream);
        outStream.close();
        ...
        ByteArrayInputStream inStream = new ByteArrayInputStream(
                outStream.toByteArray());
        ConnectResponse output = Records.deserialize(new ConnectResponse(),
                inStream, Records.CONNECT_TAG);
