...
public class FrameEncoder 
    extends MessageToMessageEncoder<BufEvent> {
    ...
    protected Object encode(ChannelHandlerContext ctx, BufEvent msg)
        ...
        if (logger.isTraceEnabled()) {
            logger.trace("Sending frame of length {} to {}", 
                    length, ctx.channel().remoteAddress());
        }
        ByteBuf header = Header.alloc(ctx.alloc());
        header.writeInt(length);
        @SuppressWarnings("unchecked")
        CallbackList<Void> callback = new CallbackList<Void>(new BufCallback(buf), msg.getCallback());
