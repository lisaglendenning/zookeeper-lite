...
public class ConnectionStateHandler extends ChannelDuplexHandler {
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        ...
        state.compareAndSet(Connection.State.OPENING, Connection.State.OPENED);
    ...
    public void channelInactive(ChannelHandlerContext ctx) throws Exception {
        ...
        state.set(Connection.State.CLOSED);
    ...
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause)
            throws Exception {
        logger.warn("Closing connection {}", ctx.channel(), cause);
        ctx.close();
    ...
    public void close(ChannelHandlerContext ctx, ChannelPromise future) throws Exception {
        if (state.get() != Connection.State.CLOSED) {
            state.set(Connection.State.CLOSING);
        }
